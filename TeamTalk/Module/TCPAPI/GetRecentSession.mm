    ...
    Analysis analysis = (id)^(NSData* data)
    {
        ...
        for (ContactSessionInfo *sessionInfo in [rsp contactSessionList]) {
            ...
            if (nRet == 0) {
                ...
                session.lastMsg= [[NSString alloc] initWithData:[GTMBase64 decodeData:[[NSString stringWithCString:pOut encoding:NSUTF8StringEncoding] dataUsingEncoding:NSUTF8StringEncoding]] encoding:NSUTF8StringEncoding];
                Free(pOut);
            }else{
                session.lastMsg=@" ";
            }
