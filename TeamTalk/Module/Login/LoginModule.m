...
- (void)autologinWithUsername:(NSString*)name password:(NSString*)password success:(void(^)(MTTUserEntity* loginedUser))success failure:(void(^)(NSString* error))failure
{
    ...
    //
    [[SessionModule instance] loadLocalSession:^(bool isok) {}];
    [self b_loadAllGroupsCompletion:nil];
 ...
 *  @param completion 异步执行的block
 */
- (void)p_loadAllGroupsCompletion:(void(^)())completion
{
    __block NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    ...
    [getFixgroup requestWithObject:nil Completion:^(NSArray *response, NSError *error) {
        if (!error)
        {
            [[MTTDatabaseUtil instance] removeAllGroup];
 ...
 *  @param completion 异步执行的block
 */
- (void)b_loadAllGroupsCompletion:(void(^)())completion
{
    [[MTTDatabaseUtil instance] loadGroupsCompletion:^(NSArray *contacts, NSError *error) {
        if ([contacts count] !=0) {
            [contacts enumerateObjectsUsingBlock:^(MTTGroupEntity *obj, NSUInteger idx, BOOL *stop) {
