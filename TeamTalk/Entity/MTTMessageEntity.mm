...
+(MTTMessageEntity *)makeMessageFromPB:(MsgInfo *)info SessionType:(SessionType)sessionType
{
    ...
    }else
    {
        ...
        if (nRet == 0) {
            msg.msgContent = [[NSString alloc] initWithData:[GTMBase64 decodeData:[[NSString stringWithCString:pOut encoding:NSUTF8StringEncoding] dataUsingEncoding:NSUTF8StringEncoding]] encoding:NSUTF8StringEncoding];
